...
export default {
  ...
  //         return v.id === this.$store.state.seller.id
  //       })
  //     })
  //   }
  // },
  ...
  // 把每一个菜品列表加入数组中
  updated () {
    this.$refs.cateItems.forEach(v => {
      let height = this.cateItemsHeight[this.cateItemsHeight.length - 1]
      height = v.offsetHeight + height
      this.cateItemsHeight.push(height)
      // console.log(this.cateItemsHeight)
    })
  },
  methods: {
    selectList (index) {
